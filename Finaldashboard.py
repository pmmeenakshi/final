...
CSV_WINDOWS_DEFAULT = Path(r"C:\Users\meena\Downloads\kaggle\FINALWORK\finaldataset.csv")
# Fallback to a local copy if you drop it next to the app
CSV_LOCAL_FALLBACK  = BASE_DIR / "finaldataset.csv"
...
def load_and_prepare_csv(csv_path: Path):
    ...
    colmap = {}
    for c in df.columns:
        if c.lower() == "lat": colmap[c] = "Lat"
        if c.lower() == "lon": colmap[c] = "Lon"
    if colmap:
...
def popup_charts_for_comm(dfl_filtered: pd.DataFrame, community_id: str):
    ...
    # CO2 donut (from dry tonnage or total tonnage)
    donut_img = ""
    dry_candidates = ["Tonnage_Dry", "Dry_Tonnage", "DryWaste", "Tonnage"]
    dry_month = None
    for m in dry_candidates:
